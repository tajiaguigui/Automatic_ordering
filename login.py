...
def empower():
    ...
    r_3 = ''.join(str(random()).split('.'))[15:19]
    print(r_3)
    print(len(r_3))
    ...
    rsp = requests.get("https://qrlogin.taobao.com/qrcodelogin/generateQRCode4Login.do?adUrl=&adImage=&adText=&viewFd4PC=&viewFd4Mobile=&from=333&appkey=00000000&umid_token=" + str(umid_token))
    ...
    #print(c)
    print(b['url'])
    success_url = b['url'] + "&umid_token=" + str(umid_token)
    print(success_url)
    #对应第 4 步
    # r = sess.get(b['url'])
    # cookies = rsp.cookies.get_dict()
